...
from prompt_toolkit.filters import has_selection   # if not already imported
...
def cli_bindings(cli: Any) -> KeyBindings:
    ...
    @kb.add("c-k", filter=~has_selection)
    def _(event):
        """Previous command in history."""
        event.current_buffer.history_backward(count=event.arg)
    
    @kb.add("c-j", filter=~has_selection)
    def _(event):
    ...
    # @kb.add("c-l", filter=~has_selection)
    # def _(event):
    ...
    @kb.add("c-l", filter=~has_selection)
    def _(event):
        """Make Ctrl+L act like the Right arrow key."""
        event.app.key_processor.feed(KeyPress(Keys.Right))        

    @kb.add("c-a", filter=~has_selection)
    def _(event):
        """Move to the beginning of the line."""
        event.current_buffer.cursor_position = 0

    @kb.add("c-e", filter=~has_selection)
    def _(event):
